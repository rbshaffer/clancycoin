...
const char * const CLANCYCOIN_CONF_FILENAME = "clancycoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "clancycoind -foo=bar
...
#else
    const char* pszModule = "clancycoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Clancycoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Clancycoin
    // Mac: ~/Library/Application Support/Clancycoin
    // Unix: ~/.clancycoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Clancycoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Clancycoin";
#else
    // Unix
    return pathRet / ".clancycoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", CLANCYCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Clancycoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Clancycoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Clancycoin Core developers";
    }
