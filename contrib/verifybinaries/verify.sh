#!/usr/bin/env bash
# Copyright (c) 2016 The Clancycoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   clancycoincore.org and clancycoin.org and compares them.
...
WORKINGDIR="/tmp/clancycoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://clancycoincore.org"
HOST2="https://clancycoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="clancycoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'clancycoin-',
...
if [ $? -ne 0 ]; then
   echo "clancycoin.org failed to provide signature file, but clancycoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "clancycoin.org and clancycoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Clancycoin Core binary release signing key installed?"
   fi
